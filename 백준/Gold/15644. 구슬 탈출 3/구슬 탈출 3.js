...
function bfs() {
  ...
  while (q.length) {
    // console.table(q);
    const [ry, rx, by, bx, cnt, path] = q.shift();
    ...
    for (let i = 0; i < 4; i++) {
      ...
      // 빨간구슬 이동
      [nry, nrx, nby, nbx, isPush, isEnd, newPath] = move(
        ry,
        rx,
        by,
        bx,
      ...
      if (isPush && cnt < 9) {
        q.push([nry, nrx, nby, nbx, cnt + 1, newPath]);
      }
    }
  }
...
// 빨간 구슬 이동
function move(ry, rx, by, bx, dir, path) {
  let nry = ry;
  let nrx = rx;
  let nby = by;
  let nbx = bx;
  ...
  if (dir === 0) {
    ...
    if (ry < by) {
      ...
      // 성공 : return [..., isPush, isEnd]
      if (isRedPossible && !isBluePossible) {
        return [nry, nrx, nby, nbx, false, true, path + "U"];
      }

      // 그냥 이동.
      return [nry, nrx, nby, nbx, true, false, path + "U"];
    } else {
      [nby, nbx, isBluePossible] = moveBlue(nry, nrx, nby, nbx, dir);
  ...
  // down
    ...
    if (ry > by) {
      ...
      // 성공 : return [..., isPush, isEnd]
      if (isRedPossible && !isBluePossible) {
        return [nry, nrx, nby, nbx, false, true, path + "D"];
      }

      // 그냥 이동.
      return [nry, nrx, nby, nbx, true, false, path + "D"];
    } else {
      [nby, nbx, isBluePossible] = moveBlue(nry, nrx, nby, nbx, dir);
  ...
  // left
    ...
    if (rx < bx) {
      ...
      // 성공 : return [..., isPush, isEnd]
      if (isRedPossible && !isBluePossible) {
        return [nry, nrx, nby, nbx, false, true, path + "L"];
      }

      // 그냥 이동.
      return [nry, nrx, nby, nbx, true, false, path + "L"];
    } else {
      [nby, nbx, isBluePossible] = moveBlue(nry, nrx, nby, nbx, dir);
    ...
    if (rx > bx) {
      ...
      // 성공 : return [..., isPush, isEnd]
      if (isRedPossible && !isBluePossible) {
        return [nry, nrx, nby, nbx, false, true, path + "R"];
      }

      // 그냥 이동.
      return [nry, nrx, nby, nbx, true, false, path + "R"];
    } else {
      [nby, nbx, isBluePossible] = moveBlue(nry, nrx, nby, nbx, dir);
