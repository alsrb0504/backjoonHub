...
let prev = 1;
input.slice(N + 1, N + 1 + query).forEach((num) => {
  const nextNode = Number(num);
  answer += LCA(prev, nextNode);
  prev = nextNode;
});
...
function LCA(x, y) {
  ...
  // 부모가 같은 경우 반환
  if (x === y) return depthArr[a] + depthArr[b] - 2 * depthArr[x];
  ...
  return depthArr[a] + depthArr[b] - depthArr[parent[x][0]] * 2;
}