...
const costs = new Array(MAX).fill(0);
...
function dfs(node, depth, accCost) {
  visited[node] = true;
  depthArr[node] = depth;
  ...
  for (let [next, cost] of graph[node]) {
    ...
    parent[next][0] = node;
    costs[next] = accCost + cost;

    dfs(next, depth + 1, costs[next]);
  }
}
...
function LCA(a, b) {
  ...
  // 부모가 같은 경우 반환
  if (x === y) return costs[a] + costs[b] - 2 * costs[x];
  ...
  return costs[a] + costs[b] - 2 * costs[parent[x][0]];
}
